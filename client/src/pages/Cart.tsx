...
export default function Cart() {
  ...
  const handleWhatsAppInquiry = () => {
    ...
    const itemsList = cart.map(item => 
      `• ${item.name} (P/N: ${item.partNumber}) - Qty: ${item.quantity}`
    ).join("\n");
    ...
    const whatsappNumber = "1234567890"; // Replace with env var in real app
                 ...
                 <div className="flex-1 p-4 flex flex-col justify-between">
                    <div className="flex justify-between items-start mb-2">
                       <div>
                         ...
                         <p className="text-sm text-muted-foreground">P/N: {item.partNumber}</p>
                       </div>
           ...
           <Card className="sticky top-24">
             <CardContent className="p-6">
               ...
               <p className="text-xs text-center mt-4 text-muted-foreground">
                 No payment is processed on this website. All transactions are handled directly with our sales team.
               </p>
             </CardContent>
           </Card>
        </div>
